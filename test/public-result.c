...
int main(void)
{
  ...
  uint8_t all_flags[MORPHEUS_ALL_MORPH_FLAG_CAPACITY];
  _Static_assert(MORPHEUS_MORPH_FLAG_GROUP_NAME <=
                 MORPHEUS_ALL_MORPH_FLAG_CAPACITY*8u,
  ...
  assert(morpheus_result_truncated_fields(NULL,0,NULL)==
         MORPHEUS_INVALID_ARGUMENT);
  assert(morpheus_result_all_morph_flags(NULL,0,all_flags,
         sizeof all_flags)==MORPHEUS_INVALID_ARGUMENT);
