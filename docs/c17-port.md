...
The `cruncher` front end plus the `anal`, `gkdict`, `gkends`, `gener`, and
`greeklib` runtime modules are strict boundaries. Their internal interfaces are
declared in `src/anal/cruncher_internal.h`, `src/anal/anal_internal.h`,
...
`src/greeklib/greeklib_internal.h`; all six targets compile with
...
1. Extend the explicit internal-interface boundary now used by `cruncher`,
   `anal`, `gkdict`, `gkends`, `gener`, and `greeklib` to `morphlib`, then
   enable
