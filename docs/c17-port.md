...
Code collation tables, and the dynamically allocated morphology-flag lookup
tables. Contexts have explicit create, activate, and destroy operations;
activation is thread-local, while the historical `set_lang` and `cur_lang`
calls remain compatible. Destruction releases both flag tables. Other mutable
caches still need to migrate before the analyser is reentrant.
