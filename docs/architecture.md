...
The first context boundary now owns the active language selection and the lazy
Beta Code collation tables. The legacy `set_lang` and `cur_lang` entry points
dispatch through an opaque runtime context, and context activation is
thread-local. Remaining caches and formatting state are still process-wide, so
this is an incremental isolation boundary rather than a thread-safety guarantee.
